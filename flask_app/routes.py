...
def add_manufacturer_redirect():
	...
	with engine_RK.connect() as con:
		con.execute('INSERT INTO manu_info (manufacturer_name) VALUES (%s);', (manu_name))
...
def kit():
	...
	return render_template('kit.html', title='Kit', kit_master=kit_master)


@app.route("/add_kit", methods=['GET', 'POST'])
def add_kit():
	with engine_RK.connect() as con:
		manu_name = con.execute('SELECT Manufacturer FROM manu_info;')
